...
const FilterSection = ({
  ...
  // Safely extract unique categories and brands, fallback to "Unknown"
  const categoryOnlyData = [
    "All",
    ...new Set(
      data.map((item) => item.category || "Unknown").filter((cat) => cat) // remove null/undefined
    ),
  ];
  const brandOnlyData = [
    "All",
    ...new Set(data.map((item) => item.brand || "Unknown").filter((b) => b)),
  ];
